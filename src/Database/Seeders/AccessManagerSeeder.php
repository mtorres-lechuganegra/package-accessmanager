...
class AccessManagerSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        $syncCustom = config('accessmanager_seeders.groups');

        // Unir ambos archivos de configuración
        $sync = array_merge($syncDefinitions, $syncCustom);

        // Recorrer la configuración y crear los permisos y rutas
        foreach ($sync as $keyGroup => $groupData) {
            $this->createPermissions($keyGroup, $groupData);
        }
    }

    /**
     * Crea los permisos y las rutas asociadas para un grupo.
     *
     * @param string $keyGroup
     * @param array $groupData
     * @return void
     */
    private function createPermissions(string $keyGroup, array $groupData)
    {
        // Formatemos el grupo
        $group = $this->formatMachineKey($keyGroup);

        // Crear los permisos
        foreach ($groupData['permissions'] as $permissionKey => $permissionData) {
